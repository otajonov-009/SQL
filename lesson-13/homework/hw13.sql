...
select top 1 cast(EMPLOYEE_ID as varchar)+'-'+FIRST_NAME+' '+LAST_NAME from Employees
 ...
 --2-task
select*, REPLACE(phone_number,'124','999') as new_number from employees
--3-task
select first_name as ism, 
len(first_name)as ism_uzunligi
from employees
...
ORDER BY 
 ...
 --8-task
select*, coalesce(ssn,passportid,itin) as info_person from person  
--9-task
SELECT*,
      LEFT(FullName, CHARINDEX(' ', FullName) - 1) AS FirstName,
       SUBSTRING(FullName,CHARINDEX(' ', FullName),CHARINDEX(' ', FullName,
...
--13-task
select*, (select count(employee_id) from Employees as emp1 where employees.department_id = emp1.department_id)from Employees
 
--14-task
select*from students
SELECT s.StudentID,
       s.fullName,
       SUM(s2.grade) AS CumulativeScore
FROM Students as s
JOIN Students as s2
  ON s2.StudentID <= s.StudentID

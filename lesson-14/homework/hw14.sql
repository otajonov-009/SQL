...
--7-task
select employee_id,first_name,last_name,hire_date, datediff(year,hire_date,getdate()) as worked_year from Employees
...
WHERE rn = 3
--11-task
SELECT EMPLOYEE_ID,FIRST_NAME,LAST_NAME,Hire_Date,DATEDIFF(YEAR, Hire_Date, GETDATE()) AS YearsWorked, 
...
FROM Employees
--12-task
SELECT Vals,LEFT(Vals, PATINDEX('%[^0-9]%', Vals + 'x') - 1) AS LeadingInteger
FROM GetIntegers   
--13-task
select*from MultipleVals
SELECT 
    Id,
    RIGHT(LEFT(Vals, CHARINDEX(',', Vals) + 1), LEN(LEFT(Vals, CHARINDEX(',', Vals) + 1)) - 2) 
    + LEFT(Vals, 1)
...
DECLARE @nums VARCHAR(100) = ''

WHILE @i <= @len
BEGIN
    IF SUBSTRING(@str, @i, 1) LIKE '[0-9]'
        SET @nums = @nums + SUBSTRING(@str, @i, 1)
    ELSE
        SET @chars = @chars + SUBSTRING(@str, @i, 1)

    SET @i = @i + 1
END;
SELECT @chars AS Letters, @nums AS Numbers
